...
import { Subject, takeUntil, filter, forkJoin, take, Observable } from "rxjs";
...
const contactPersonIdentitiesPayload = formValue.identities?.map((i: any) => {
      const entry: any = {
    ...
    const contactPersonPhoneTypesPayload = formValue.phoneTypes?.map((i: any) => {
      const entry: any = {
...
console.log('route',this.route.snapshot)
  ...
  this.router.navigate(
    ['/crm/clients/view-contact-person', clientIdParam]
  );
} else {

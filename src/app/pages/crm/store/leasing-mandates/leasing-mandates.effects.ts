...
import { LeasingMandatesService } from './leasing-mandates.service';
...
@Injectable()
export class LeasingMandatesEffects {
  constructor(private actions$: Actions, private svc: LeasingMandatesService) {}
  ...
  create$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ payload }) => {
        const dto = payload as Omit<LeasingMandate, 'id'>;
        return this.svc.create(dto).pipe(
          mergeMap((entity) => [
            ActionsList.createEntitySuccess({ entity }),
            ActionsList.entityOperationSuccess({
              entity: EntityNames.LeasingMandate,
              operation: 'create',
            }),
          ]),

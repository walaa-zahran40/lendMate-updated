...
export class AddInterestRateBenchmarksComponent {
  ...
  viewOnly = false;
  addInterestRateBenchmarkLookupsForm!: FormGroup;
  clientId: any;
  ...
  ngOnInit() {
    ...
    // 1. Build the form
    this.addInterestRateBenchmarkLookupsForm = this.fb.group({
    ...
    console.log(
      ...
      this.addInterestRateBenchmarkLookupsForm.value
    );
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      if (idParam) {
        ...
        if (this.viewOnly) {
          this.addInterestRateBenchmarkLookupsForm.disable();
        ...
        this.facade.selected$
          ...
          .subscribe((ct) => {
            this.addInterestRateBenchmarkLookupsForm.patchValue({
              id: ct!.id,
              name: ct!.name,
              nameAR: ct!.nameAR,
  ...
  addOrEditInterestRateBenchmark() {
    console.log('💥 addOrEditInterestRateBenchMark() called');
    console.log('  viewOnly:', this.viewOnly);
    console.log('  editMode:', this.editMode);
    console.log(
      '  form valid:',
      this.addInterestRateBenchmarkLookupsForm.valid
    );
    console.log(
      '  form touched:',
      this.addInterestRateBenchmarkLookupsForm.touched
    );
    console.log(
      '  form raw value:',
      this.addInterestRateBenchmarkLookupsForm.getRawValue()
    );

    // Print individual control errors
    const nameCtrl = this.addInterestRateBenchmarkLookupsForm.get('name');
    const nameARCtrl = this.addInterestRateBenchmarkLookupsForm.get('nameAR');
    ...
    if (this.addInterestRateBenchmarkLookupsForm.invalid) {
      console.warn('❌ Form is invalid — marking touched and aborting');
      this.addInterestRateBenchmarkLookupsForm.markAllAsTouched();
      return;
    }

    const { name, nameAR, isActive } =
      this.addInterestRateBenchmarkLookupsForm.value;
    const payload: Partial<InterestRateBenchMark> = { name, nameAR, isActive };
    ...
    if (this.editMode) {
      const { id, name, nameAR, isActive } =
        this.addInterestRateBenchmarkLookupsForm.value;
      const payload: InterestRateBenchMark = { id, name, nameAR, isActive };
      console.log(

...
import * as ActionsList from './asset-type-categories.actions';
...
export class AssetTypeCategoriesEffects {
  constructor(
    ...
    private svc: AssetTypeCategoriesService
  ) {}
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.svc.getAll().pipe(
          ...
          catchError((err) => {
            console.error('⚠️ Error loading assetTypeCategories', err);
  ...
  create$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ payload }) => {
        const dto = payload as Omit<AssetTypeCategory, 'id'>;
        return this.svc.create(dto).pipe(
          mergeMap((entity) => [
            ActionsList.createEntitySuccess({ entity }),
            ActionsList.entityOperationSuccess({
              entity: EntityNames.AssetTypeCategory,
              operation: 'create',
            }),
          ]),
  ...
  refreshList$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(() => ActionsList.loadAssetTypeCategoryHistory())
    )
  );
  // Load area history
  loadAssetTypeCategory$ = createEffect(() =>
    this.actions$.pipe(
      ofType(ActionsList.loadAssetTypeCategoryHistory),
      switchMap(() =>
        this.svc.getAllHistory().pipe(
          map((history) =>
            ActionsList.loadAssetTypeCategoryHistorySuccess({ history })
          ),
          catchError((error) =>
            of(ActionsList.loadAssetTypeCategoryHistoryFailure({ error }))
          )
        )
      )
    )

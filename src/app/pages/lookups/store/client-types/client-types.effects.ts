...
import * as ActionsList from './client-types.actions';
...
export class ClientTypesEffects {
  constructor(private actions$: Actions, private svc: ClientTypesService) {}
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.svc.getAll().pipe(
          ...
          catchError((err) => {
            console.error('⚠️ Error loading client-action-types', err);
  ...
  create$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ payload }) => {
        const dto = payload as Omit<ClientType, 'id'>;
        return this.svc.create(dto).pipe(
          mergeMap((entity) => [
            ActionsList.createEntitySuccess({ entity }),
            ActionsList.entityOperationSuccess({
              entity: EntityNames.ClientType,
              operation: 'create',
            }),
          ]),
  ...
  refreshList$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(() => ActionsList.loadClientTypeHistory())
    )
  );
  // Load authorization group officer history
  loadClientTypeHistory$ = createEffect(() =>
    this.actions$.pipe(
      ofType(ActionsList.loadClientTypeHistory),
      switchMap(() =>
        this.svc.getAllHistory().pipe(
          map((history) =>
            ActionsList.loadClientTypeHistorySuccess({ history })
          ),
          catchError((error) =>
            of(ActionsList.loadClientTypeHistoryFailure({ error }))
          )
        )
      )
    )

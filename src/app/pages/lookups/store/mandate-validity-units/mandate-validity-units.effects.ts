...
export class MandateValidityUnitsEffects {
  constructor(
    ...
    private service: MandateValidityUnitsService
  ) {}
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.service.getAll().pipe(
          ...
          catchError((err) => {
            console.error('⚠️ Error loading mandateValidityUnits', err);
  ...
  create$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ payload }) => {
        // payload is Partial<Omit<MandateValidityUnit,'id'>>, but our service needs the full DTO shape
        const dto = payload as Omit<MandateValidityUnit, 'id'>;

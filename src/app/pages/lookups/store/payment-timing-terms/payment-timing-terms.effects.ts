...
export class PaymentTimingTermsEffects {
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.svc.getAll().pipe(
          ...
          catchError((err) => {
            console.error('⚠️ Error loading paymentTimingTerms', err);
  ...
  create$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ payload }) => {
        // payload is Partial<Omit<PaymentTimingTerm,'id'>>, but our service needs the full DTO shape
        const dto = payload as Omit<PaymentTimingTerm, 'id'>;

...
@Injectable()
export class TmlOfficerTypesEffects {
  constructor(private actions$: Actions, private svc: TmlOfficerTypesService) {}
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.svc.getAll().pipe(
          ...
          catchError((err) => {
            console.error('⚠️ Error loading tmlOfficerTypes', err);
  ...
  create$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ payload }) => {
        // payload is Partial<Omit<TmlOfficerType,'id'>>, but our service needs the full DTO shape
        const dto = payload as Omit<TmlOfficerType, 'id'>;

...
import * as ActionsList from './roles.actions';
...
export class RolesEffects {
  constructor(private actions$: Actions, private service: RolesService) {}
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.service.getAll().pipe(
          ...
          catchError((err) => {
            console.error('⚠️ Error loading roles', err);
  ...
  create$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ payload }) => {
        const dto = payload as Omit<Role, 'id'>;
        return this.service.create(dto).pipe(
          mergeMap((entity) => [
            ActionsList.createEntitySuccess({ entity }),
            ActionsList.entityOperationSuccess({
              entity: EntityNames.Role,
              operation: 'create',
            }),
          ]),

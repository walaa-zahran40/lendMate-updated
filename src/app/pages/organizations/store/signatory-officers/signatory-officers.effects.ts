...
import { SignatoryOfficersService } from './signatory-officers.service';
import * as ActionsList from './signatory-officers.actions';
...
export class SignatoryOfficersEffects {
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.svc.getAll().pipe(
          ...
          catchError((err) => {
            console.error('⚠️ Error loading signatoryOfficers', err);
  ...
  create$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ payload }) => {
        const dto = payload as Omit<SignatoryOfficer, 'id'>;

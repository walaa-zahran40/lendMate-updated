...
import * as ActionsList from './teams.actions';
...
export class TeamsEffects {
  constructor(private actions$: Actions, private svc: TeamsService) {}
  ...
  create$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ payload }) => {
        const dto = payload as Omit<Team, 'id'>;
        return this.svc.create(dto).pipe(
          mergeMap((entity) => [
            ActionsList.createEntitySuccess({ entity }),
            ActionsList.entityOperationSuccess({
              entity: EntityNames.Team,
              operation: 'create',
            }),
          ]),
